...
use crate::particle_system::{ParticleSimulator, create_particle_system};
...
pub struct ParticleApp {
    particle_system: Box<dyn ParticleSimulator>,
...
impl ParticleApp {
    pub fn new(cc: &eframe::CreationContext<'_>, compute_supported: bool) -> Self {
        // When creating the particle system, use the factory function:
        let wgpu_render_state = cc.wgpu_render_state.as_ref().unwrap();
        let device = &wgpu_render_state.device;

        // Load compute shader if supported
        let compute_shader = if compute_supported {
        ...
        // Always load the fragment shader for fallback
        let fragment_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Fragment Update Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/fragment_update.wgsl").into()),
        });
        ...
        // Create appropriate particle system
        let particle_system =
            create_particle_system(device, compute_shader.as_ref(), &fragment_shader, is_web);

        // Create camera and renderer as before
        // let camera = Camera::new(device, initial_width as f32 / initial_height as f32);
        ...
        let renderer = ParticleRenderer::new(device, &camera, &surface_format, &vertex_shader);
    ...
    fn update_simulation(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        if let Some(wgpu_render_state) = frame.wgpu_render_state() {
            ...
            self.particle_system.set_mouse_dragging(self.mouse_dragging);
            ...
            if self.mouse_dragging {
                ...
                let current_pos = self.particle_system.get_mouse_position();

                let current_pos = glam::Vec3::new(current_pos[0], current_pos[1], current_pos[2]);
                ...
                self.particle_system
                    .set_mouse_position([world_pos.x, world_pos.y, world_pos.z]);
            }

            // Update particles with compute shader if not paused
            if !self.particle_system.is_paused() {
    ...
    fn render_ui(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        egui::Window::new("Particle Simulator")
            ...
            .show(ctx, |ui| {
                ...
                let paused = self.particle_system.is_paused();
                if ui.button(if paused { "Resume" } else { "Pause" }).clicked() {
                    self.particle_system.set_paused(!paused);
                }
                ...
                let mouse_pos = self.particle_system.get_mouse_position();
                ui.label(format!(
                    "Position: ({:.2}, {:.2}, {:.2})",
                ...
                ui.label(format!("Depth: {:.2}", mouse_pos[2]));

                let mut mouse_radius = self.particle_system.get_mouse_radius();
                ui.add(egui::Slider::new(&mut mouse_radius, 1.0..=50.0).text("Radius"));
                self.particle_system.set_mouse_radius(mouse_radius);

                let mut mouse_force = self.particle_system.get_mouse_force();
                ui.add(egui::Slider::new(&mut mouse_force, 0.0..=100.0).text("Force"));
                self.particle_system.set_mouse_force(mouse_force);
                ...
                let mut num_particles = self.particle_system.get_num_particles();
                ui.horizontal(|ui| {
                    ui.label("Count: ");
                    let response = ui.add(
                        egui::DragValue::new(&mut num_particles)
                            .speed(1000)
                            .range(1..=self.particle_system.get_max_particles()),
                    );

                    if response.changed() {
                        self.particle_system.set_num_particles(num_particles);
                    }
                });

                ui.horizontal(|ui| {
                    if ui.button("10,000").clicked() {
                        self.particle_system.set_num_particles(
                            10_000.min(self.particle_system.get_max_particles()),
                        );
                    }
                    if ui.button("100,000").clicked() {
                        self.particle_system.set_num_particles(
                            100_000.min(self.particle_system.get_max_particles()),
                        );
                    }
                    if ui.button("1,000,000").clicked() {
                        self.particle_system.set_num_particles(
                            1_000_000.min(self.particle_system.get_max_particles()),
                        );
                    }
                });

                // Display mode section
                let mut color_mode = self.particle_system.get_color_mode();
                egui::ComboBox::from_label("Color Mode")
                    .selected_text(match color_mode {
                        0 => "Original",
                        1 => "Velocity",
                        2 => "Position",
                        _ => "Unknown",
                    ...
                    .show_ui(ui, |ui| {
                        ui.selectable_value(&mut color_mode, 0, "Original");
                        ui.selectable_value(&mut color_mode, 1, "Velocity");
                        ui.selectable_value(&mut color_mode, 2, "Position");
                    });
                self.particle_system.set_color_mode(color_mode);
    ...
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        ctx.input(|input| {
            ...
            if input.raw_scroll_delta.y != 0.0 {
                ...
                let camera_forward = self.camera.get_forward();
                let current_pos = self.particle_system.get_mouse_position();
                let current_pos = Vec3::new(current_pos[0], current_pos[1], current_pos[2]);

                let move_distance = scroll_delta * 0.2; // Adjust sensitivity
                let new_pos = current_pos + camera_forward * move_distance;
                self.particle_system
                    .set_mouse_position([new_pos.x, new_pos.y, new_pos.z]);
            }
        });
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let callback_obj = UnsafeParticleCallback {
                ...
                particle_buffer_ptr: self.particle_system.get_particle_buffer() as *const _,
                num_particles: self.particle_system.get_num_particles(),
            };
